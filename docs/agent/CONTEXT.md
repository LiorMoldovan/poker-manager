...
> **Last refreshed**: 2026-05-10 (post v5.59.0 — chip-counting full rebuild: per-stack pipeline + multi-method voting + selfies + detection overlay + pipeline-meta diagnostics)

---

## Right now

- **Version on `origin/main`**: `5.59.0` — full rebuild of photo chip counting after the multi-shot consensus approach (v5.49) was producing confidently-wrong results with no diagnostics. Replaced with: per-stack divide-and-conquer (1 LLM call per stack), 3 deterministic geometric methods (bottom-chip self-cal, gradient counting, cross-stack shared-cal), weighted voting via `combineLLMAndGeometry`, client-side stack region detection (white-stripe primary → edge density fallback → position-only last resort), white-balance correction from sampled mat color, chip selfies as few-shot LLM reference + calibrated HSL palette, total-value sanity check (live-game only, off-by-N-chips reconciliation), `ChipDetectionOverlay` component with bounding-box visualization, `pipeline_meta` JSONB on feedback rows + per-stack provenance log. Bundled fixes: removed `gemini-2.5-pro` (was burning quota then forcing weaker fallbacks → `parseFailed`), corrected sign-inversion bug in dashboard + tuner (positive bias is "AI undercounting", was labeled the opposite). Bundled non-pipeline work: trivia `countTemplates('group')` shows group-only count so the "all = group + players" math reads correctly, CSS spinner-arrow hide on number inputs (chips row was crowding 4-digit values against new 📷 buttons). Removed the photo-share opt-in card from Settings (provenance log makes failure replay possible without the original photo; plumbing kept for future debug toggle, no group has it enabled).
- **No in-flight WIP**: working tree clean after the v5.59.0 merge push.
- **Applied SQL migrations** (both v5.59 ones live):
  ...
  - **One-time data hygiene applied during the v5.59 ship**: deleted 2 stale feedback rows in Lior's group (`d1998bed-…`) from the previous single-shot model so the dashboard + auto-tuner start the new pipeline with a clean baseline. No tuning overrides existed (no rollback needed). Storage bucket `chip-count-feedback-photos` was already empty (`share_chip_photos` was never enabled by any group).
