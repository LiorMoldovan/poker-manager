...
## 2026-05-10 — Photo chip-counting full rebuild + multi-agent merge (v5.59.0)

**Asked**: After a long session producing the rebuild plan and implementation, "merge changes from all agents and bump, please also send me short instructions to Eyal since the mat and all chips are with him i want him to train the model, it should be short simple and in hebrew in few bullets". Earlier in the same session, Lior had explicitly demanded a holistic non-patch solution: "i am not looking for patches or quick and dirty solutions, i am looking for deep analysis for the full feature with proper correction that will really work… do proffesional work without patches and work around".

**Did**:
- **Architectural rebuild of `countChipsFromPhoto`** — replaced the v5.49 multi-shot consensus (which was producing confidently-wrong counts and a `parseFailed` cascade after `gemini-2.5-pro` quota burns) with a per-stack divide-and-conquer pipeline. New flow: client-side stack region detection → mat color sample + white-balance correction → parallel per-stack work (LLM with chip-selfie reference + 3 deterministic geometric methods) → weighted vote → optional total-value sanity check.
- **New modules**: `src/utils/stackDetection.ts` (white-stripe density primary, edge-density fallback, position-only last resort, color-to-chip mapping via HSL distance against selfie palette), `src/utils/chipGeometry.ts` (bottom-chip self-cal, gradient counting, cross-stack shared-cal, vote aggregation), `src/components/ChipDetectionOverlay.tsx` (bounding-box visualization with per-stack labels + adjusted-stack highlighting).
- **New schema**: `supabase/074-chip-selfies.sql` (selfie_base64 + selfie_dominant_hex on chip_values), `supabase/075-chip-feedback-pipeline-meta.sql` (pipeline_meta JSONB on chip_count_feedback). Both applied live.
- **Augmented existing modules**: `imageUtils.ts` got `decodeImageStreaming`, `sampleMatColor`, `cropToRegion`, `sampleDominantColor`, `whiteBalanceFromStripes`, `applyWhiteBalance`, RGB↔HSL↔hex utilities, and `captureChipSelfie` (single-chip optimized capture). `geminiAI.ts` lost the obsolete multi-shot helpers (`runChipCountShot`, `combineShots`, `computeStackConfidence`, etc.) and gained `runSingleStackShot`, `combineLLMAndGeometry`, `applyTotalValueSanityCheck`. `chipFeedbackStats.ts` gained pipeline-aware aggregator KPIs (pctVerifiedByMultipleMethods, avgAgreementScore, detectionSignalCounts, pctWhiteBalanceApplied, totalValueAdjustmentsFired).
- **UI integration**: `ChipEntryScreen` got the collapsible detection overlay in the AI banner, "↻ Clear AI suggestions" link (preserves per-field user edits), 🛟 "adjusted" badge when total-value sanity check fired. `SettingsScreen` Chips tab got the per-color 📷 selfie button + thumbnail + clear, with an orange tip banner when selfies are missing. `SettingsScreen` test card was rebuilt with the detection overlay + per-stack provenance tooltips. Dashboard added the "🛠️ pipeline health" KPI block.
- **Removed at user request**: the "Chip-count feedback opt-in" card in Settings ("if i dont need it remove it, if i need it keep it" — provenance log replaces failure replay, all groups had `share_chip_photos = false` so no behavior change).
- **Bundled fixes from parallel agents**: trivia `countTemplates('group')` showing group-only count so "all = group + players" math is consistent (`triviaGenerator.ts`), CSS spinner-arrow hide on `.input[type=number]` (the new 📷 buttons were crowding 4-digit values in the chips row).
- **Data hygiene**: deleted 2 stale feedback rows from Lior's group from the previous single-shot model so the dashboard + auto-tuner start from a clean baseline. Verified no tuning overrides existed (no rollback needed). Storage bucket `chip-count-feedback-photos` was already empty.
- **Validation pass before merge** (8-item checklist): manual flow byte-identical when no photo (verified via `!!stack` gating on all new icons), per-field user edits preserved on apply + clear, selfie round-trip end-to-end (DB columns verified live), pipeline reads `selfieBase64` + `selfieDominantHex` with both branches handled, feedback row + dashboard contract sound (legacy rows skip new KPIs cleanly), RLS unchanged (chip_count_feedback admin SELECT via 071 still in force, chip_values admin all-ops via cv_admin), no new localhost-vs-production gap (`/api/gemini` path identical), `npx tsc --noEmit` clean, ReadLints clean across all 13 touched files.
- **Bumped to v5.59.0** (folded the parallel agent's pending `5.58.1` patch entry into the rebuild changelog since parseFailed + sign-inversion were subsumed). Pushed to `origin/main`.

**Learned**:
- The user's emotional investment ("i have high hopes") combined with the "1000000% accuracy" original ask is a setup for disappointment. Calibrated expectations explicitly in the wrap-up: 60–80% per-stack accuracy on first photo with no selfies, 75–90% with selfies, meaningful tuner gains only after 20–30 feedback rows. Better to under-promise upfront than recover trust after disappointment.
- The rebuilt pipeline is genuinely better-architected (per-stack decomposition, multi-signal voting, deterministic geometric methods, white balance, selfie grounding, provenance log) but represents the **best free-tier-compatible CV approach** rather than a guaranteed-accurate solution. A trained CNN/YOLO would beat it; we don't have labeled training data.
- The auto-tuner only changes the **per-stack LLM prompt** — it cannot tune the geometric methods, stack detection, or color mapping (those are deterministic code). User asked about this directly; explained the boundary clearly so they don't expect the loop to fix code-level bugs.
- Removing UI cards the user explicitly questions ("do i really need this 2?") + giving them an immediate one-line rationale for the removal worked well. Trying to "preserve every feature" is a clutter trap.
- When inheriting parallel-agent work via uncommitted file mods (CSS spinner fix from one agent + trivia template-count fix from another + a pending v5.58.1 changelog entry), bundling everything under the bigger version bump with explicit "bundled fixes from parallel work" lines in the commit body is cleaner than 3 separate commits.

**Next**:
- Eyal to take chip selfies (one per color) on the real green felt mat with the real chip set, run 10–20 test photos through Settings → photo test card with manual ground-truth corrections submitted as feedback. Lior to then run "Tune Now" and verify the auto-rollback safety net engages if the tune underperforms.
- If geometric counting underperforms on monochrome chips (no stripes for `gradient-count` to leverage), consider adding a fourth geometric method based on chip body color uniformity.
- Watch the dashboard's `detectionSignal` breakdown — if `position-only` shows up frequently, stack detection needs camera-framing guidance in the modal (separation distance, angle, lighting tips).
