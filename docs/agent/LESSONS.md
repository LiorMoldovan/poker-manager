...
## 2026-05-15 — `auth.uid() IS NULL` guards asymmetrically reject service-role callers

**Incident**: Lior reported getting emails for every vote-change on Eyal's poll (instructed push-only). Same Settings screen showed the EmailJS Usage card at 122/200 while the EmailJS dashboard read 195/200 — a 73-email gap. Root cause: two bugs (one in the dispatch layer, one in the audit-log layer) that compounded. The audit-log bug is the structural one worth remembering: `log_email_send` (migration 052, SECURITY DEFINER, granted to `authenticated`) had `if auth.uid() is null then raise exception 'unauthenticated'` as defense-in-depth. When v5.49.0 introduced the server-side notification worker (`api/notification-worker.ts`), worker-mode requests forwarded `Bearer SUPABASE_SERVICE_ROLE_KEY` to `/api/send-email`, which then called `log_email_send` with that header. Service-role calls have NO `auth.uid()` — it returns NULL — so the guard raised on every worker dispatch. The `try/catch` in `send-email.ts` swallowed the exception silently (`console.error(...)`); the email went out via EmailJS, the audit row was never inserted, and the Usage card under-counted by ~30% for 5 days.

**Root cause**: I (or whoever wrote migration 052) treated `auth.uid() IS NULL` as a "is the caller authenticated?" check. That intuition is correct for **user-JWT-only** RPCs, but the function ended up callable from a **second context** (service-role from Edge Functions) where `auth.uid()` is structurally NULL regardless of how legitimate the caller is. The guard didn't distinguish "anon caller with the public anon key" (which we want to reject) from "Edge Function with service-role" (which we want to accept). Compounded by the silent `try/catch` that ate the rejection — without it, the bug would have surfaced as a 4xx in the send-email logs and been caught immediately.

**Lesson**: When a SECURITY DEFINER RPC is callable from both user-JWT and service-role contexts, **never use `auth.uid() IS NULL` as the auth check**. The function is already gated by `grant execute … to authenticated, service_role` and runs as a privileged owner — that's the security boundary. If you need defense-in-depth, gate at the API layer (here: `verifyAuth` in `api/_auth.ts` already rejects unauthenticated callers via worker-secret OR user-JWT). For the RPC body itself, either drop the check entirely or use a positive marker (e.g. parameter validity, group membership lookup). Companion lesson: ALWAYS check the postgres return when calling an RPC from an Edge Function — `fetch().ok` is true on 200 OR on a non-2xx body, but the response body carries the `code: '42501'` that tells you the RPC raised. `try/catch` around the fetch only catches network errors, not application errors that returned a 4xx. The fix shape is `if (!r.ok) console.warn(await r.text())` — turns silent failures into loud ones during development.

**Session**: 2026-05-15 (v5.61.1 — vote_change push-only + email-log service-role fix; migration 079).
