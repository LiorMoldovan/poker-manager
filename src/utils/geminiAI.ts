...
${gameHistoryText}`;
  ...
  const prompt = `אתה פרשן פוקר מקצועי - חכם, מצחיק וציני. כתוב תחזית קצרה וקולעת.
...
1. סכום אפס: סכום expectedProfit = 0 בדיוק!

2. highlight - הסיבה העיקרית לתחזית (10-15 מילים):
   - ציין את הנתון הכי חשוב שהשפיע על התחזית
   - אל תחזור על אותו נתון לכל השחקנים!
   - דוגמאות טובות: "3 הפסדים ברצף לאחרונה", "ממוצע +45₪ ב-10 משחקים אחרונים", "לא שיחק 3 חודשים"
   - דוגמאות רעות: "מפסיד ללא הפסקה" (לא ספציפי), "שחקן טוב" (לא נתון)

3. sentence - משפט בידורי (20-35 מילים):
   - מצחיק, ציני, או דרמטי
   - קשור לשחקן הספציפי
   - לא לחזור על אותו סגנון!
   - אם אותו שחקן מנצח את כולם - מספיק לציין פעם אחת, לא בכל שחקן!

4. היגיון בריא:
   - אם שחקן אחד מנצח את כולם - ציין רק אצלו או אצל היריב הכי משמעותי
   - לא לכתוב "מפסיד ל-X" על כל שחקן!
   - מצא את ההייליט הכי מעניין לכל אחד - משהו אחר!
   - גיוון: כל שחקן מקבל זווית שונה

5. מגדר: לנקבה (מור) - היא, שלה, ניצחה. לזכר - הוא, שלו, ניצח.

6. expectedProfit ריאלי לפי הטווח ההיסטורי של השחקן.
