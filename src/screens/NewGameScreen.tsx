...
const NewGameScreen = () => {
  ...
  const generateCreativeSentence = (
    ...
    switch (expectedOutcome) {
      ...
      case 'neutral':
        const neutralSentences = [
          `${name} הוא חידה מוחלטת הערב - הנתונים לא נותנים שום רמז לאיזה כיוון הוא ילך`,
          `50-50 עבור ${name} - יכול להיות ערב מדהים או ערב לשכוח. תלוי באיזה ${name} יגיע`,
          `${name} על קו האפס - השאלה הגדולה היא לאיזה צד הוא ייפול, ואף אחד לא יודע`,
          `${name} בלתי צפוי לחלוטין - זה מה שמעניין בו, אף פעם לא יודעים מה יקרה`,
          `מי יודע מה יעשה ${name} הלילה? הנתונים לא עוזרים, והכל תלוי במזל ובמצב רוח`,
          `${name} יכול להפתיע לכל כיוון - גם ניצחון גדול וגם הפסד כואב אפשריים לגמרי`,
        ];
        return neutralSentences[Math.floor(Math.random() * neutralSentences.length)];
        
      case 'slight_loss':
        const slightLossSentences = [
          `${name} צריך קצת מזל הלילה - הנתונים לא לגמרי לצידו, אבל זה לא אומר שהוא לא יכול להפוך את הקערה`,
          `הרוח לא לגמרי לטובת ${name} הערב - הוא יצטרך להילחם על כל שקל אם הוא רוצה לצאת ברווח`,
          `${name} מתחיל עם חיסרון קל - לא דרמטי, אבל מספיק כדי להקשות על הערב`,
          `הסיכויים קצת נגד ${name} - הוא יצטרך משחק טוב במיוחד כדי להפוך את המגמה`,
          `${name} בא לערב מאתגר - הנתונים מציעים שזה לא יהיה קל, אבל הכל אפשרי`,
          `${name} יצטרך לעבוד קשה הלילה - היתרון לא לצידו, אבל שחקנים טובים יודעים איך להתגבר`,
        ];
        return slightLossSentences[Math.floor(Math.random() * slightLossSentences.length)];
        
      case 'loss':
        const lossSentences = [
          `${name} לא בעמדה הכי טובה הערב - הנתונים מראים שהוא יצטרך הרבה מזל כדי לשנות את המגמה`,
          `קשה להיות אופטימי לגבי ${name} הלילה - הסטטיסטיקות לא לצידו ונראה שיהיה לו קשה`,
          `${name} בא עם רוח נגדית חזקה - אולי כדאי לו לשחק שמרני ולא לקחת סיכונים`,
          `הנתונים לא מבשרים טובות ל${name} - נראה שזה הולך להיות ערב מאתגר במיוחד`,
          `${name} יצטרך נס קטן כדי לסיים ברווח - הסיכויים ממש לא לצידו הערב`,
          `${name} לא במיטבו על פי הנתונים - השאלה היא האם הוא יצליח להפתיע למרות הכל`,
        ];
        return lossSentences[Math.floor(Math.random() * lossSentences.length)];
        
      case 'big_loss':
        const bigLossSentences = [
          `${name} הוא הספונסר הלא רשמי של הקבוצה - בזכותו כולנו מרוויחים יותר, ועל זה אנחנו מודים`,
          `תודה מראש ל${name} על התרומה - הנתונים מראים שהוא כנראה יחלק כסף לכולם הלילה`,
          `${name} בא לבלות עם חברים, לא לנצח - וזה בסדר גמור, כי מישהו צריך לממן את המשחק`,
          `${name} מוכיח שפוקר זה לא רק על כסף - הוא בא ליהנות, גם אם הארנק שלו סובל`,
          `כולם אוהבים כש${name} מגיע - בעיקר הארנקים שלהם. הנתונים מדברים בעד עצמם`,
    ...
    return `${name} - ערב מסקרן מחכה לו, נראה מה יקרה`;
  };

  // NEW PLAYERS - No history - LONGER engaging sentences
  const newPlayerSentences = [
    `{name} נכנס למשחק בלי היסטוריה - דף חלק לגמרי. הלילה נתחיל לכתוב את הסיפור שלו`,
    `אין לנו שום מידע על {name} - הוא יכול להיות גאון פוקר או הפסד מובטח. הלילה נגלה`,
    `{name} הוא סוס אפל מוחלט - בלי נתונים, בלי היסטוריה, בלי שום רמז למה שיקרה`,
    `מי זה בכלל {name}? אין לנו מושג, ובדיוק זה מה שמעניין. הכל פתוח`,
    `{name} מתחיל מאפס - בלי יתרון ובלי חיסרון. הערב הזה יקבע את הרושם הראשוני`,
    `שחקן חדש בשם {name} מצטרף למשחק - בואו נראה מה הוא עשוי ממנו`,
    `{name} בלי תיק עבר - יכול להפתיע את כולם לטובה או לרעה. מי יודע?`,
  ];
